...
You might have noticed that this repo comes with two separate compose files. By default, both `docker-compose.yml` and `docke-compose.override.yml` are run in which the configuration for the `pelican` and `webserver` containers from `docker-compose.yml` are added to the configuration for the jupyter service from `docker-compose.override.yml`. If you're interested in reading more about compose overrides, you can do so [here](https://docs.docker.com/compose/extends/).
